...
class Scenario:
    ...
    def _is_far_enough_from_other_goals(self, x: int, y: int, min_distance: int) -> bool:
        ...
        for goal_x, goal_y in self._goals:
...
class NearestGoalScenario(Scenario):
    ...
    def generate_goals(self) -> None:
        ...
        while len(self._goals) < self.num_goals and attempts < max_attempts:
        ...
        if len(self._goals) < self.num_goals:
            raise RuntimeError(f"요청한 {self.num_goals}개의 목표 지점 중 {len(self._goals)}개만 생성할 수 있었습니다.")
