...
def visualize_state(grid_world, state, step, agent=None):
    """
    현재 상태를 시각화합니다.
    ...
    # 현재 상태의 최적 행동 표시
    if agent and state in agent.pi:
...
def visualize_agent_movement(grid_world, agent, max_steps=30):
    """
    ...
    while not done and step < max_steps:
        # 현재 상태 시각화
        visualize_state(grid_world, state, step, agent)
    ...
    print(f"에이전트가 {f'목표에 도달했습니다. 총 스텝 수: {step}' if done else '최대 스텝 수에 도달했습니다.'}")